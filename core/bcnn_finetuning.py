...
class vgg16:
    ...
    def fc_layers(self):
        ...
        with tf.variable_scope('fc-new') as scope:
            fc3w = tf.get_variable('W', [512*512, 50], initializer=tf.contrib.layers.xavier_initializer(), trainable=True)
            #fc3b = tf.Variable(tf.constant(1.0, shape=[100], dtype=tf.float32), name='biases', trainable=True)
            fc3b = tf.get_variable("b", [50], initializer=tf.constant_initializer(0.1), trainable=True)
...
def _parse_function(example_proto):
    ...
    image = tf.image.resize_images(
        image_decoded, size)
    label = tf.one_hot(parsed_features["image/class/label"],50)
    return image,label
...
if __name__ == '__main__':
    ...
    train_data_names = ["/data1/zhaoweiqiang/workspace/models/research/slim/dfdata/deepfasion/deepfasion_train.tfrecord"]
    ...
    for step in range(1000000):
        ...
        print("Step:", '%03d' % step, "Loss:", str(cost), "Accuracy:", accuracy.eval(session=sess),end='\r')
