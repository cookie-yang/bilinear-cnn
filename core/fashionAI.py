...
def _parse_function(example_proto):
    keys_to_features = {
        ...
        "image/class/label": tf.FixedLenFeature([5],tf.int64)
    }
...
if __name__ == '__main__':
    ...
    lr = tf.train.exponential_decay(
        0.005, global_step, decay_steps=2000, decay_rate=0.96, staircase=True)
    optimizer_fc = tf.train.MomentumOptimizer(
        learning_rate=lr, momentum=0.9).minimize(loss, var_list=fc_paramters)
    ...
    for step in range(20000):
        ...
        if step % saving_interval == 0:
            print("saved model")
            saver.save(sess,'./model/fashionAI/fashionAI.ckpt')
        train_accuracy += accuracy.eval(session=sess)
        vgg.imgs = imgs_vali
        vali_accuracy += accuracy.eval(session=sess)
        print("Step:{0:03d} Loss:{1:.5f} Vali_accuracy:{2:.3f} Train_accuracy:{3:.3f}".format(
            step, cost/(step+1), vali_accuracy/(step+1), train_accuracy/(step+1)),end='\r')
        vgg.imgs = imgs
